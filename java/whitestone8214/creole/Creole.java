...
public class Creole extends Activity {
	...
	protected void onCreate(Bundle state) {
		...
		/* Apply window setting(s). */
		requestWindowFeature(Window.FEATURE_NO_TITLE);
		getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
		
		/* Get necessary size(s). */
		int _idStatusbar = getResources().getIdentifier("status_bar_height", "dimen", "android");
		int _heightStatusbar = getResources().getDimensionPixelSize(_idStatusbar);
		_widthApp = getWindowManager().getDefaultDisplay().getWidth();
		_heightApp = getWindowManager().getDefaultDisplay().getHeight() - _heightStatusbar;
		//_spanIcon = (_widthApp < _heightApp) ? (_widthApp / 30) : (_heightApp / 30);
		_spanIcon = _heightApp / 30;
		
		/* Initialize some necessary things. */
		...
		_helperDraw = new DrawHelper(_widthApp, _heightApp, _nGridsInALine);
		
		/* Top-level window. */
		_overall = new LinearLayout3(this).label("folder").width(_widthApp).height(_heightApp).fixedSize(true).direction(1).
			looks(new Looks() {public void action(View _this, Canvas canvas) {canvas.drawColor(0xffffffff);}});
			
		/* Toolbar. */
		_toolbar = new LinearLayout3(this).width(_widthApp).height(_spanIcon * 2).
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				LinearLayout3 _this1 = (LinearLayout3)_this;
				...
				_helperDraw.rectangle(canvas, _spanIcon * 2, _spanIcon / 10, _this1.width - (_spanIcon * 2), (_spanIcon * 2) - (_spanIcon / 10), 0, 0, 0xff000000, false);
				String _here = "?";
				...
				try {
					_helperDraw.text(canvas,
						_widthApp - (_spanIcon * 4) - ((_spanIcon / 5) * 2), _spanIcon * 2,
						(_spanIcon * 2) + (_spanIcon / 5), _spanIcon * 1.375f,
				...
				/* Menu button */
				_helperDraw.icon(canvas, _this1.width - (_spanIcon * 2), 0, 0xff000000, "3x3");
			}}).
			in(buttonIcon(100, "zoneselect")).
			in(buttonIcon(200, "address")).
			in(buttonIcon(100, "zonemenu"));
		_overall.in(_toolbar);
		
		/* File manager */
		_boxFiles3 = zone_base("/").
			packing(_enginePackingLinear).touchReaction(_engineScroll).snatchReaction(_engineTap);
			
		/* Text editor */
		_boxDocument3 = zone_base("none").
			packing(_enginePackingLinear).
			in(
				new EditText2(this).width(_widthApp).height(_heightApp - _spanIcon).
					textColor(0xff000000).textSize((_spanIcon / 4) * 3).alignment(Gravity.LEFT | Gravity.TOP).
					looks(new Looks() {public void action(View _this, Canvas canvas) {canvas.drawColor(0xffffffff);}})
			);
			
		/* Image viewer */
		_boxPicture3 = zone_base("none").
			packing(_enginePackingLinear).touchReaction(_engineScroll).snatchReaction(_engineTap);
			
		/* Music player */
		_boxMusic = zone_base("none").
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				canvas.drawColor(0xffffffff);
			}}).
			in(menuitem2("music-title")).
			in(menuitem2("music-artist")).
			in(menuitem2("music-album"));
			
		/* Web browser */
		_boxWeb3 = zone_base("about:blank").
			in(new WebView2(this).width(_widthApp).height(_heightApp - _spanIcon));
			WebView2 _boxWeb = (WebView2)_boxWeb3.getChildAt(0);
			_clientWebChrome = new WebChromeClient();
			_boxWeb.setWebChromeClient(_clientWebChrome);
			_clientWebGeneral = new WebViewClient();
			_boxWeb.setWebViewClient(_clientWebGeneral);
			_boxWeb.setDownloadListener(new DownloadListener() {public void onDownloadStart(String path, String profile, String description, String type, long length) {
				try {
					((DownloadManager)getApplicationContext().getSystemService(Context.DOWNLOAD_SERVICE)).enqueue(
						new DownloadManager.Request(Uri.parse(path)).setDestinationUri(Uri.parse("file:///sdcard/Download/" + new File(new URI(path).toURL().getFile()).getName()))
					);
					Toast.makeText(getApplicationContext(), "Download: " + path, 1).show();
				}
				catch(Exception x) {}
			}});
			
		/* Test zone */
		_boxTest = zone_base("none");
			//loadTest("/sdcard");
	...
	public void onBackPressed() {
		...
		_dialog1.clear().label("ask-overall-quit").
			in(textLine(_widthApp - (_spanIcon * 2), "Quit?")).
			...
			if (_here.compareTo("folder") == 0) {
				...
				try {
					...
					_boxPicture3.clear().x(0).y(0).
						in(new LinearLayout3(this).width(_widthPicture).height(_heightPicture).fixedSize(true).
							looks(new Looks() {public void action(View _this, Canvas canvas) {canvas.drawBitmap(_picture, 0, 0, new Paint());}})
						);
	...
	private LinearLayout3 zone_base(String label) {
		return new LinearLayout3(this).label(label).width(_widthApp).height(_heightApp - _spanIcon).fixedSize(true).direction(1);
	}
	private TextView2 empty_base(String label, int width, int height) {return new TextView2(this).label(label).width(width).height(height);}
	private TextView2 buttonIcon(int type, String value) {
		return empty_base(value, (type == 200) ? (_widthApp - (_spanIcon * 4)) : (_spanIcon * 2), _spanIcon * 2).
			whenClicked(new View.OnClickListener() {public void onClick(View _this) {
				...
				if (_this1.label.compareTo("zoneselect") == 0) {
					try {dismissDialog(0);} catch(Exception x) {}
					
					_dialog1.clear().
						in(menuitem2("zone-files")).in(menuitem2("zone-document")).in(menuitem2("zone-picture")).in(menuitem2("zone-music")).in(menuitem2("zone-web")).
						in(menuitem2("zone-test")).
						in(menuitem2("close"));
						
					showDialog(0);
				}
				else if (_this1.label.compareTo("address") == 0) {
					try {dismissDialog(0);} catch(Exception x) {}
							
					if (_overall.label.compareTo("folder") == 0) {
						_dialog1.clear().label("ask-files-path").
							in(entryGeneral(_widthApp - (_spanIcon * 2), _valueFilesHere)).
						...
						_dialog1.clear().label("ask-document-path").
							in(entryGeneral(_widthApp - (_spanIcon * 2), (_valueDocumentHere != null) ? _valueDocumentHere : "")).
							in(stick(3, "v-dialog", "Load", null)).
							in(stick(3, "x-dialog", "Cancel", null).index(10));
					}
					else if (_overall.label.compareTo("mountain") == 0) {
						_dialog1.clear().label("ask-picture-path").
							in(entryGeneral(_widthApp - (_spanIcon * 2), (_valuePictureHere != null) ? _valuePictureHere : "")).
							in(stick(3, "v-dialog", "Load", null)).
							in(stick(3, "x-dialog", "Cancel", null).index(10));
					}
					...
					/* File manager */
						...
						if (_id.endsWith("new") == true) {
							...
							_dialog1.clear().label("ask-files-create").
								in(entryGeneral(_widthApp - (_spanIcon * 2), _valueFilesHere + "/new")).
							...
							_dialog1.clear().label("ask-files-paste").
								in(textLine(_widthApp - (_spanIcon * 2), ((_stateToDo == 1) ? "Copy " : "Move ") + Integer.toString(_listFilesToDo.size()) + " file(s) here?")).
								in(stick(3, "v-dialog", "Yes", null)).
								in(stick(3, "x-dialog", "No", null).index(10));
							...
							_dialog1.clear().label("ask-files-delete").
								in(textLine(_widthApp - (_spanIcon * 2), "Delete " + Integer.toString(_nFilesSelected) + " selected file(s)?")).
								in(stick(3, "v-dialog", "Yes", null)).
								in(stick(3, "x-dialog", "No", null).index(10));
					...
					/* File manager (Context menu) */
						...
						if (_id.endsWith("internal") == true) {
							...
							_dialog1.clear().label("ask-files-copy1").
								in(entryGeneral(_widthApp - (_spanIcon * 2), _valueFileContext + "-new")).
							...
							_dialog1.clear().label("ask-files-delete1").
								in(textLine(_widthApp - (_spanIcon * 2), "Delete this file?")).
								in(stick(3, "v-dialog", "Yes", null)).
								in(stick(3, "x-dialog", "No", null).index(10));
					...
					/* Text editor */
						...
						if (_id.endsWith("new") == true) {
							...
							_dialog1.clear().label("ask-document-new").
								in(textLine(_widthApp - (_spanIcon * 2), "Unsaved data will be lost.")).
							...
							_dialog1.clear().label("ask-document-load-encrypted").
								in(entryGeneral(_widthApp - (_spanIcon * 2), null)).
								in(entryMasked(_widthApp - (_spanIcon * 2), "0000")).
								in(entryGeneral(_widthApp - (_spanIcon * 2), "")).
								in(stick(3, "v-dialog", "Load", null)).
								in(stick(3, "x-dialog", "Cancel", null).index(10));
							...
							_dialog1.clear().label("ask-document-save").
								in(entryGeneral(_widthApp - (_spanIcon * 2), (_valueDocumentHere != null) ? _valueDocumentHere : "/sdcard/new")).
					...
					/* Image viewer */
						...
						else if (_id.endsWith("photo") == true) {
							startActivity(new Intent(getApplicationContext(), CameraRoom.class));
					...
					/* Music player */
						...
						else if (_id.endsWith("video") == true) {
							try {_music.pause();} catch(Exception x) {}
							
							Intent _worker = new Intent(getApplicationContext(), VideoRoom.class);
							_worker.setData(Uri.parse("content://" + _valueMusicHere));
							startActivity(_worker);
					...
					_dialog1.clear().label("menu-files-context").
						in(textLine(_widthApp - (_spanIcon * 2), new File(_id).getName())).
	...
	private EditText2 entryGeneral(int width, String valueDefault) {
		EditText2 _entry = new EditText2(this).width(width).height(_spanIcon * 2).text(valueDefault).oneline(true).
				looks(new Looks() {public void action(View _this, Canvas canvas) {
					canvas.drawColor(0xffbfff00);
				}});
			_entry.setTextSize(TypedValue.COMPLEX_UNIT_PX, _spanIcon);
			_entry.setTextColor(0xff000000);
			_entry.setPadding(_spanIcon / 5, 0, _spanIcon / 5, 0);
	...
	private TextView2 textLine(int width, String text) {
		TextView2 _text = new TextView2(this).width(_widthApp - (_spanIcon * 4)).height(_spanIcon * 2).text(text).
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				TextView2 _this1 = (TextView2)_this;
				Paint _drawer = new Paint();
				_drawer.setFlags(Paint.ANTI_ALIAS_FLAG);
				_drawer.setColor(0xffffffff);
				_drawer.setTextSize(_spanIcon);
				canvas.drawText(_this1.getText().toString(), _spanIcon / 5, _spanIcon * 1.375f, _drawer);
			}});
	...
	private LinearLayout3 stick(int type, String icon1, String label, String icon2) {
		...
		*/
		...
		/* Overall */
		LinearLayout3 _stick = new LinearLayout3(this).width(_widthApp).height(_spanIcon * 2).
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				LinearLayout3 _this1 = (LinearLayout3)_this;
			...
			if (type == 3) {
				_stick.in(
					new TextView2(this).label(label).width(_widthApp - (_spanIcon * 2)).height(_spanIcon * 2).
						looks(new Looks() {public void action(View _this, Canvas canvas) {
							TextView2 _this1 = (TextView2)_this;
							Paint _drawer = new Paint();
							_drawer.setFlags(Paint.ANTI_ALIAS_FLAG);
							_drawer.setColor(0xffffffff);
							_drawer.setTextSize(_spanIcon);
							canvas.drawText(_this1.label, _spanIcon / 5, _spanIcon * 1.375f, _drawer);
						}}).
						whenClicked(new View.OnClickListener() {public void onClick(View _this) {
							TextView2 _this1 = (TextView2)_this;
				...
				_stick.in(
					new TextView2(this).label(label).width(_widthApp - (_spanIcon * 2)).height(_spanIcon * 2).text(_name).
						looks(new Looks() {public void action(View _this, Canvas canvas) {
							TextView2 _this1 = (TextView2)_this;
							Paint _drawer = new Paint();
							_drawer.setFlags(Paint.ANTI_ALIAS_FLAG);
			...
			if (icon2 != null) {
				if (type == 1) {
					...
					_label.width(_label.width - (_spanIcon * 2));
				}
				else {
					TextView2 _label = (TextView2)_stick.getChildAt(icon1 != null ? 1 : 0);
					_label.width(_label.width - (_spanIcon * 2));
	...
	private LinearLayout3 menuitem2(String id) {
		return new LinearLayout3(this).label(id).width(_widthApp - (_spanIcon * 2)).height(_spanIcon * 2).
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				LinearLayout3 _this1 = (LinearLayout3)_this;
				...
				String _icon = null; String _label = null;
					...
					else if (_this1.label.compareTo("picture-photo") == 0) {_icon = "3x3"; _label = "Take photo";}
					...
					else if (_this1.label.compareTo("music-video") == 0) {_icon = "3x3"; _label = "View video";}
	...
	private LinearLayout3 fileitem2(String path) {
		return new LinearLayout3(this).label(path).width(_widthApp - (_spanIcon * 2)).height(_spanIcon * 2).
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				LinearLayout3 _this1 = (LinearLayout3)_this;
				...
				try {
					...
					_helperDraw.text(canvas,
						_widthApp - (_spanIcon * 4), _spanIcon * 2,
						(_spanIcon * 2) + (_spanIcon / 5), _spanIcon * 1.375f,
						(_file.isDirectory() == true) ? 0xffbf3f3f : 0xff000000,
						_file.getName(), Paint.Align.LEFT, true
					);
					_helperDraw.icon(canvas, _widthApp - (_spanIcon * 2), 0, (_file.isDirectory() == true) ? 0xffbf3f3f : 0xff000000, "3x3");
				}
				catch(Exception x) {}
			}}).
