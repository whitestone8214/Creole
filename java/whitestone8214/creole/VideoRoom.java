...
public class VideoRoom extends Activity {
	...
	protected void onCreate(Bundle state) {
		...
		try {
			Uri _path = getIntent().getData();
			if (_path.getPath().startsWith("/media") != true) _path = Uri.parse("file://" + _path.getPath());
			
			_video = new MediaPlayer();
			_video.setDataSource(_path.getPath());
			_video.prepare();
			_widthVideo = _video.getVideoWidth();
			_heightVideo = _video.getVideoHeight();
			setRequestedOrientation((_widthVideo > _heightVideo) ? ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE : ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
		}
		catch(Exception x) {x.printStackTrace(); finish();}
	...
	public void onBackPressed() {
		...
		showDialog(0);*/
		try {_video.reset(); _video.release();} catch(Exception x) {}
		finish();
	}
	...
	void chapter2(View _this, int widthNatural, int heightNatural) {
		...
		_overall = ((LinearLayout3)_this).label("folder").width(_display.getWidth()).height(_display.getHeight()).fixedSize(true).direction(1).
			looks(new Looks() {public void action(View _this, Canvas canvas) {
				LinearLayout3 _this1 = (LinearLayout3)_this;
				
				canvas.drawColor(0xff000000);
			}}).
			packing(new Packing() {public void action(View _this, boolean changed, int left, int top, int right, int bottom) {
				LinearLayout3 _this1 = (LinearLayout3)_this;
				SurfaceView2 _child = null;
				try {_child = (SurfaceView2)_this1.getChildAt(0);} catch(Exception x) {return;}
				
				int _x = (_this1.width - _child.width) / 2;
				int _y = (_this1.height - _child.height) / 2;
				_child.layout(_x, _y, _x + _child.width, _x + _child.height);
			}});
		...
		_boxVideo = new SurfaceView2(this).width(_overall.width).height(_overall.height);
			_boxVideo.setOnClickListener(new View.OnClickListener() {public void onClick(View _this) {
				try {
					if (_video.isPlaying() == true) _video.pause();
					else _video.start();
				}
				catch(Exception x) {}
			}});
		_overall.in(_boxVideo);
		
		try {
			/*float _aspectWidth = ((float)_overall.width) / ((float)_widthVideo);
			float _aspectHeight = ((float)_overall.height) / ((float)_heightVideo);
			if (_widthVideo > _heightVideo) {
				if (_aspectWidth > _aspectHeight) {
					_heightVideo = _overall.height;
					float _aspect = ((float)_video.getVideoHeight()) / ((float)_video.getVideoWidth());
					float _width = ((float)_overall.width) * _aspect;
					_widthVideo = (int)_width;
				}
				else {
					_widthVideo = _overall.width;
					float _aspect = ((float)_video.getVideoHeight()) / ((float)_video.getVideoWidth());
					float _height = ((float)_overall.height) * _aspect;
					_heightVideo = (int)_height;
				}
			}
			else {
				if (_aspectHeight > _aspectWidth) {
					_widthVideo = _overall.width;
					float _aspect = ((float)_video.getVideoWidth()) / ((float)_video.getVideoHeight());
					float _height = ((float)_overall.height) * _aspect;
					_heightVideo = (int)_height;
				}
				else {
			...
			float _aspectScreen = ((float)_overall.width) / ((float)_overall.height);
			float _aspectVideo = ((float)_widthVideo) / ((float)_heightVideo);
			if (_widthVideo > _heightVideo) {
				/*if (_widthVideo > _overall.width) _widthVideo = _overall.width;
				float _aspect = ((float)_video.getVideoHeight()) / ((float)_video.getVideoWidth());
				float _height = ((float)_overall.width) * _aspect;
				_heightVideo = (int)_height;*/
				if (_aspectScreen > _aspectVideo) {
					_heightVideo = _overall.height;
					float _width = ((float)_heightVideo) * _aspectVideo;
					_widthVideo = (int)_width;
				}
				else {
					_widthVideo = _overall.width;
					float _height = ((float)_widthVideo) / _aspectVideo;
					_heightVideo = (int)_height;
				}
			}
			else {
				/*if (_heightVideo > _overall.height) _heightVideo = _overall.height;
				float _aspect = ((float)_video.getVideoWidth()) / ((float)_video.getVideoHeight());
				float _width = ((float)_overall.height) * _aspect;
				_widthVideo = (int)_width;*/
				if (_aspectScreen > _aspectVideo) {
					_widthVideo = _overall.width;
					float _height = ((float)_widthVideo) / _aspectVideo;
			...
			_boxVideo.width(_widthVideo).height(_heightVideo);
			Log.e("Creole", "BOX_VIDEO_SIZE " + Integer.toString(_boxVideo.width) + " " + Integer.toString(_boxVideo.height));
			_boxVideo.requestLayout();
			//_overall.requestLayout();
		}
		catch(Exception x) {}
		try {_video.prepare();} catch(Exception x) {}
		_boxVideo.getHolder().addCallback(new SurfaceHolder.Callback() {
			public void surfaceCreated(SurfaceHolder holder) {
				try {_video.setDisplay(holder);} catch(Exception x) {}
			}
			public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {}
			public void surfaceDestroyed(SurfaceHolder holder) {}
		});
		try {_video.start();} catch(Exception x) {}
	}
